...
set plugins => {
    Database => {
        driver   => 'SQLite',
        database => 'C:\Users\Anamul Hasan\Atachment\Sqlite\myapp.db',
...
my $url = 'http://localhost:5000/login';
my $headers = [
...
my $request = HTTP::Request->new(post => $url, $headers, $json_data);
...
#  git config --global user.email "u1804118@student.cuet.ac.bd"    
# git config --global user.name "AnamulHasanRony" 